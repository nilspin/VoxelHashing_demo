...
void buildLinearSystem(const float4* input, const float4* target, const float4* target_normal, float* linearSystem) {
	...
	const int SYSTEM_SIZE = 26;	//we need 26 floats to build Ax=b system
	const int WINDOW_SIZE = 8;	//we process 8 correspondences per thread
	extern __shared__ float linearSystem_shared[];
	...
	for (int t = 0; t < WINDOW_SIZE; ++t) {
		...
		if (isValid(n)) {
			...
			for (int i = 0; i < 6; ++i) {
				...
				linearSystem_shared[SYSTEM_SIZE*threadIdx.x + 20 + i] += A[i] * b;	//For ATb
			}
		}
	}
	__syncthreads();
	...
	float* d_generatedMatrixSystem)
{
	...
	buildLinearSystem <<<blocks, threads , 300*SYSTEM_SIZE*sizeof(float)>>>(d_input, d_target, d_targetNormals, d_generatedMatrixSystem);
